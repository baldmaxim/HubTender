...
func (f *fakeStore) rolloutState() *fakeRolloutState {
	if f.rs == nil {
...
func (f *fakeStore) TransitionRolloutMode(_ context.Context, _, expectedFrom, target, updatedBy string) (*repository.AIFeatureSettings, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (f *fakeStore) EmergencyOff(_ context.Context, _, updatedBy string) (string, *repository.AIFeatureSettings, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	...
	f.row.RolloutConfigVersion++
	f.row.UpdatedBy = &updatedBy
	return prev, f.snap(), nil
}

func (f *fakeStore) UpdateRolloutSettings(_ context.Context, _ string, p repository.AIRolloutSettingsPatch, updatedBy string) (*repository.AIFeatureSettings, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (f *fakeStore) SetLastLiveEvaluation(_ context.Context, _, evalID string) error {
	f.mu.Lock()
	defer f.mu.Unlock()
	id := evalID
...
func (f *fakeStore) ListPilotUsers(context.Context, string) ([]repository.AIPilotUser, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	rs := f.rolloutState()
...
func (f *fakeStore) UpsertPilotUser(_ context.Context, feature, userID string, bulk bool, expiresAt *time.Time, addedBy string) (*repository.AIPilotUser, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	rs := f.rolloutState()
...
func (f *fakeStore) PatchPilotUser(_ context.Context, _, userID string, patch repository.AIPilotPatch) (*repository.AIPilotUser, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	rs := f.rolloutState()
...
func (f *fakeStore) RemovePilotUser(_ context.Context, _, userID string) error {
	f.mu.Lock()
	defer f.mu.Unlock()
	rs := f.rolloutState()
...
func (f *fakeStore) GetActivePilotMembership(_ context.Context, _, userID string) (*repository.AIPilotUser, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	rs := f.rolloutState()
	p, ok := rs.pilots[userID]
...
func (f *fakeStore) CountActivePilotUsers(ctx context.Context, feature string) (int, error) {
...
func (f *fakeStore) GetCircuit(context.Context, string) (*repository.AICircuitState, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (f *fakeStore) CircuitAllowProbe(context.Context, string) (bool, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	rs := f.rolloutState()
...
func (f *fakeStore) CircuitRecordSuccess(context.Context, string) error {
	f.mu.Lock()
	defer f.mu.Unlock()
	rs := f.rolloutState()
	now := time.Now()
...
func (f *fakeStore) CircuitRecordFailure(_ context.Context, _, code string, threshold, cooldownSeconds int) (*repository.AICircuitState, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	rs := f.rolloutState()
...
func (f *fakeStore) CircuitReset(context.Context, string) (*repository.AICircuitState, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	rs := f.rolloutState()
	rs.circuit.State = "closed"
...
func (f *fakeStore) ReserveUsage(_ context.Context, in repository.AIReservationInput) (*repository.AIReservation, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	rs := f.rolloutState()
...
func (f *fakeStore) ReconcileUsage(_ context.Context, o repository.AIUsageOutcome) error {
	f.mu.Lock()
	defer f.mu.Unlock()
	rs := f.rolloutState()
...
func (f *fakeStore) RecoverExpiredReservations(context.Context, string) (int64, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	rs := f.rolloutState()
	var n int64
...
func (f *fakeStore) GetUserQuotaState(_ context.Context, _, userID string) (*repository.AIUserQuotaState, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	rs := f.rolloutState()
	dayStart := time.Now().UTC().Truncate(24 * time.Hour)
...
func (f *fakeStore) GetUsageSummary(context.Context, string) (*repository.AIUsageSummary, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	rs := f.rolloutState()
...
func (f *fakeStore) CleanupExpiredUsage(context.Context, string, time.Duration, int) (int64, error) {
	return 0, nil
}

func (f *fakeStore) InsertFeedbackSkeletons(_ context.Context, rows []repository.AIFeedbackSkeleton) error {
	f.mu.Lock()
	defer f.mu.Unlock()
	rs := f.rolloutState()
	for _, r := range rows {
...
func (f *fakeStore) ListFeedbackSkeletons(_ context.Context, requestID, userID string) ([]repository.AIFeedbackSkeletonRow, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	rs := f.rolloutState()
...
func (f *fakeStore) FinalizeFeedbackOutcomes(_ context.Context, userID string, outcomes []repository.AIFeedbackOutcome) error {
	f.mu.Lock()
	defer f.mu.Unlock()
	rs := f.rolloutState()
...
func (f *fakeStore) InsertEvaluationSummary(_ context.Context, s *repository.AIEvaluationSummary) (string, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	rs := f.rolloutState()
	rs.evalSeq++
...
func (f *fakeStore) ListEvaluationSummaries(context.Context, string, int) ([]repository.AIEvaluationSummary, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	rs := f.rolloutState()
...
func (f *fakeStore) GetEvaluationSummary(_ context.Context, id string) (*repository.AIEvaluationSummary, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	rs := f.rolloutState()
