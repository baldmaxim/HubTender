...
// qualityServicer — граница сервиса для handler'а.
type qualityServicer interface {
	TenderQuality(ctx context.Context, tenderID string) (*quality.Report, error)
}

// QualityHandler — GET /api/v1/tenders/{id}/quality (та же auth-политика, что
...
func (h *QualityHandler) TenderQuality(w http.ResponseWriter, r *http.Request) {
