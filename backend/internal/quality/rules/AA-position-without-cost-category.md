...
- FP: ожидаются позиции, которые сознательно держат вне разреза (например,
  временные или служебные строки).

## Замер

Не снят. При замере полезно сразу посчитать суммарный `money_delta` по базе — он
показывает, какая доля денег сегодня не видна в разрезе категорий, и это же
число оправдывает или отменяет правило.
