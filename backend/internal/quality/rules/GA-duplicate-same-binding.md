...
одинаковой привязке к работе, одинаковой детальной категории затрат и одинаковых
коэффициентах перевода и расхода**. Разные коэффициенты под одной работой — это разные
слои или операции (утеплитель 50 + 100 мм = 150 мм по наименованию строки заказчика),
а не задвоение.
...
-- Коэффициенты перевода и расхода — часть ключа: один материал под одной работой
-- с разными коэффициентами — это разные слои или операции (утеплитель 50 + 100 мм,
-- уайт-спирит на грунтовку и на окраску), а не задвоение.
GROUP BY b.tender_id, cp.position_number, cp.item_no, b.client_position_id,
         ...
         b.conversion_coefficient, b.consumption_coefficient
HAVING COUNT(*) > 1
...
- FP (отсечены 2026-09-16): «Большая Татарская» v2, поз. 795 и 1630–1632 — ТЕХНОФАС
  ОПТИМА двумя слоями, коэффициенты перевода 0.05 и 0.1 (в сумме 150 мм, как в строке
  заказчика); поз. 124, 126, 818, 819 — уайт-спирит с коэффициентами 0.0079 и 0.4878.
  Все 8 групп Татарской — этот класс, проверяющий подтвердил.
...
После добавления коэффициентов в ключ (2026-09-16): **1 350 групп в 52 тендерах**,
0.82 млрд ₽; на «Большой Татарской» v2 — 0.
