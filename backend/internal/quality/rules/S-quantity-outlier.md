...
Для **счётных единиц** (шт, компл, комплект, комплекс, к-т, ед) порог — 1000×, а не 100×:
«1 шт» позиции — одна конструкция, а «шт» материала — её детали. Витраж «1 шт» законно
содержит 100–240 шт профиля и крепежа.
...
WHERE n.pos_unit <> ''
  ...
  -- Счётные единицы: «1 шт» позиции — одна конструкция, а «шт» материала — её
  -- детали (витраж 1 шт = 100–240 шт профиля, «Большая Татарская»). Для них выброс —
  -- только от 1000× («1 компл» позиции и 18 415 компл материала).
  AND n.quantity > CASE
        WHEN n.pos_unit IN ('шт', 'шт.', 'компл', 'компл.', 'комплект', 'комплекс', 'к-т', 'ед', 'ед.')
          THEN 1000
        ELSE 100
      END * n.manual_volume
...
- FP (отсечены 2026-09-16): «Большая Татарская» v2 — 11 витражных конструкций «1 шт»
  со 104–240 шт деталей; проверяющий отметил все как норму. По базе в том же классе —
  «144 шт на 1 шт», «4 916 шт на 14 шт».
...
База 105 тендеров: порог 100× для всех единиц — 181 находка в 47 тендерах; с порогом
1000× для счётных единиц — **87 в 18**. На «Большой Татарской» v2: 11 → 0. Находки с
кратностью ~18 000× («Расходники на системы вентиляции») остаются.
