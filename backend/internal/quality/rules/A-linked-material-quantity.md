...
SELECT
  ...
  cp.item_no,
  b.id AS entity_id,
  md5(concat_ws('|', trim_scale(b.quantity), trim_scale(w.quantity),
                trim_scale(b.conversion_coefficient),
                trim_scale(b.consumption_coefficient),
                trim_scale(b.unit_rate))) AS fingerprint,
  concat_ws(' ',
...
- TP: «Большая Татарская», поз. 811/812 — материал ровно 10× формулы (~25.5 млн ₽ из
  65 находок A). Выносится в черновик **AB**; после его `active` эти строки из A
  исключить.
...
- FP-кандидат: «Большая Татарская», мягкие A без кратности 10× (формула от работы,
  ГП от объёма позиции / ручной ввод) — ~50 из 65; лечить вердиктом, не пересчётом.
...
Замер-срез (2026-09-16): «Большая Татарская» — 65 A / ~35.9 млн ₽, из них ≈12 с
кратностью 10× (~25.5 млн). См. [BACKLOG-TATARSKAYA-2026-09](../../../../docs/data-quality/BACKLOG-TATARSKAYA-2026-09.md).
