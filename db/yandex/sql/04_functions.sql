-- =============================================================================
-- 04_functions.sql — application SQL/PLpgSQL functions.
--
-- Source: supabase/migrations/00000000000005_baseline_functions.sql
--         + 00000000000014_fn_save_redistribution_results.sql.
--
-- CLEANING APPLIED:
--   * No Supabase-internal dependencies. The only external symbol is auth.uid(),
--     which on Yandex is the compatibility shim from
--     01_auth_compat_or_app_auth.sql (reads app.user_id / app.current_user_id
--     session GUC; returns NULL when unset). Per-function auth.uid() handling is
--     documented in docs/yandex-migration/07_SCHEMA_BUILD_REPORT.md §Functions.
--   * The 7 functions that PROD migration 10 patched via a dynamic ALTER
--     FUNCTION loop now carry `SET search_path` inline (handle_updated_at,
--     auto_archive_tender_registry, auto_create_tender_registry,
--     get_positions_with_costs, trg_boq_items_update_grand_total,
--     trg_markup_pct_update_grand_total, trg_subcontract_excl_update_grand_total).
--   * public.notify_row_change() is NOT here — it lives in 07_pgnotify.sql.
--   * The Supabase `GRANT EXECUTE ... TO authenticated` on
--     save_redistribution_results is intentionally dropped (no Supabase roles;
--     see 08_permissions.sql).
--   * pg_notify channel `rowchange` is preserved (07); the Supabase/PostgREST
--     `pgrst` reload channel is NOT used by Go BFF and is not ported.
-- =============================================================================
...
-- Fixed in PROD: stale `role` -> role_code; returns text now.
...
-- ---------------------------------------------------------------------------
-- Восстановлено из pre-prod Supabase (ocauafggjrqvopxjihas) — функция была
-- пропущена исходной DB-миграцией (см. docs/yandex-migration/26).
-- Адаптация для Yandex (предписана 03_tables.sql:13-16: no schema-qualified
-- extension calls): extensions.uuid_generate_v4() -> gen_random_uuid()
-- (ровно 2 вхождения: tmp_cp_map, tmp_boq_map). Остальное — вербатим.
-- Заголовок идентичен execute_version_transfer (выше).
