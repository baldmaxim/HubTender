...
> Этап 2.5 добавил реальный server-only OpenRouter-adapter к этому интерфейсу
> и административную страницу выбора/теста модели —
> [OPENROUTER_AI_ADMINISTRATION.md](OPENROUTER_AI_ADMINISTRATION.md).
> Пользовательские live-вызовы управляются controlled rollout этапа 2.6
> ([CONTROLLED_AI_ROLLOUT.md](CONTROLLED_AI_ROLLOUT.md)): по умолчанию off,
> live — только allowlisted-пилотам после live evaluation gate.
...
Только явное действие пользователя (кнопка); батчи по 8 строк (этап 2.6:
live-гейт показал, что 15 строк не проходят живые ZDR-endpoint'ы —
таймаут/лимит выходных токенов); ≤200 строк на
