...
Та же, что у остального портала:

```bash
TOKEN=$(curl -s -X POST "$BASE/api/v1/auth/login" \
  -H 'Content-Type: application/json' \
  -d '{"email":"service@example.com","password":"..."}' | jq -r .access_token)

curl -s "$BASE/api/v1/archive/positions/search?q=..." -H "Authorization: Bearer $TOKEN"
```

Per-tender ACL в BFF сейчас нет: авторизованный пользователь видит позиции любого тендера.
Если архив должен уважать видимость проектов — это отдельная доработка сервисного слоя
(код `ARCHIVE_SOURCE_FORBIDDEN`).
