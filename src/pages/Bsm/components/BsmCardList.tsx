...
/** Одна карточка позиции. memo: item-объекты стабильны между фильтрациями
 *  (filter не клонирует), поэтому при вводе в поиск перерисовываются только
 *  реально изменившиеся карточки, а не все 40+ разом. */
const BsmCard = memo(({ item }: { item: BoqItemData }) => {
...
BsmCard.displayName = 'BsmCard';

/** Карточный (read-only) вид БСМ для телефона в портрете. memo: при срочном
 *  рендере кейстрока (см. useDeferredValue в Bsm) props не меняются — список
 *  пропускается целиком, перестройка уезжает в deferred-проход. */
export const BsmCardList: React.FC<BsmCardListProps> = memo(({ items, loading }) => {
  ...
  return (
    <Space direction="vertical" size={8} style={{ width: '100%' }}>
      {visible.map((item) => <BsmCard key={item.id} item={item} />)}
      {hasMore && <div ref={sentinelRef} style={{ height: 1 }} />}
    </Space>
  );
});
BsmCardList.displayName = 'BsmCardList';
