...
export const useConstructionCost = () => {
  ...
  const deleteItem = (record: TreeNode) => {
    confirm({
      ...
      content: (
        <div>
          <p>Вы уверены, что хотите удалить &laquo;{record.structure}&raquo;?</p>
          {record.type === 'detail' && (
            <p>
              Вместе с детализацией каскадом удалятся связанные с ней объёмы по тендерам,
              исключения роста субподряда, псевдонимы импорта номенклатур и эталонные
              диапазоны стоимости.
            </p>
          )}
        </div>
      ...
      onOk: async () => {
        try {
          ...
          // Массовое удаление идёт по одной детализации: часть строк могла уйти
          // до отказа, поэтому дерево перечитываем и в ветке ошибки.
          await loadData();
        }
      },
    });
