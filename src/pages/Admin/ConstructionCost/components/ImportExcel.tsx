...
export const ImportExcel: React.FC<ImportExcelProps> = ({
  ...
  // knownUnits передаётся явно: после авто-добавления новых единиц повтор идёт с
  // расширенным списком. Если опираться на проп availableUnits, повторный разбор
  // читает его из старого замыкания и диалог «найдены новые единицы» возвращается
  // снова, сколько бы раз единицы ни добавляли.
  const runImport = async (file: UploadFile, knownUnits: string[]) => {
    setUploading(true);
    setImportErrors([]);
      ...
      reader.onload = async (e) => {
        try {
          ...
          rows.forEach((row, index) => {
            ...
            if (!knownUnits.includes(catUnit)) {
              unknownUnits.add(catUnit);
            }

            const detUnit = String(detailUnit).trim();
            if (!knownUnits.includes(detUnit)) {
            ...
            confirm({
              ...
              onOk: async () => {
                try {
                  ...
                  await runImport(file, [...knownUnits, ...newUnits]);
                } catch (error) {
                  console.error('Ошибка добавления единиц:', error);
                  message.error(`Не удалось добавить новые единицы измерения: ${getErrorMessage(error)}`);
                  setUploading(false);
                }
              },
              onCancel: () => {
                const sqlInserts = newUnits.map((unit, index) => {
                  ...
                  return `('${unit}', '${unit}', 'импортированная', ${nextSortOrder})`;
                }).join(',\n  ');

                const sqlQuery = `-- SQL для добавления новых единиц измерения в таблицу units
-- Выполните этот запрос в Yandex Managed PostgreSQL перед повторным импортом

INSERT INTO public.units (code, name, category, sort_order)
VALUES
  ${sqlInserts}
...
ORDER BY sort_order;`;
                ...
                setImportErrors([
                  ...
                  'Выполните предложенный SQL запрос в базе данных, затем повторите импорт',
                  ...errors
                ]);
                setUploading(false);
              }
  ...
  const handleExcelImport = (file: UploadFile) => {
    void runImport(file, availableUnits);
    return false;
  };
