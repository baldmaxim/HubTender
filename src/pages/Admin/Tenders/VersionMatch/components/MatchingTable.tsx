...
/**
 * Цвет тега для типа сопоставления
 */
function getMatchTypeColor(matchType: MatchPair['matchType']) {
  switch (matchType) {
    case 'auto':
      return 'green';
    case 'manual':
...
/**
 * Текст для типа сопоставления
 */
function getMatchTypeText(matchType: MatchPair['matchType']) {
  switch (matchType) {
    case 'auto':
      return 'Авто';
    case 'manual':
...
/**
 * Иконка для типа сопоставления
 */
function getMatchTypeIcon(matchType: MatchPair['matchType']) {
  switch (matchType) {
    case 'auto':
 ...
 *
 * Отображает старые позиции (слева), новые (справа), оценку совпадения и действия.
 * Список виртуализирован (`virtual`) — при переключении вкладок-фильтров монтируются
 * только видимые строки, поэтому даже для 500–2000+ позиций переключение мгновенное.
 */
function MatchingTableInner({
  matches,
  newPositions,
  filter,
  onToggleTransfer,
  ...
  // Высота таблицы для виртуализации (number обязателен). Адаптивно под высоту окна.
  const [tableHeight, setTableHeight] = useState(() =>
    Math.max(360, (typeof window !== 'undefined' ? window.innerHeight : 800) - 420)
  );

  useEffect(() => {
    const onResize = () => setTableHeight(Math.max(360, window.innerHeight - 420));
  ...
  // Общий список опций для Select — считается один раз, переиспользуется во всех строках
  // (раньше newPositions.map(...) вызывался в render на каждую несопоставленную строку).
  const newPositionOptions = useMemo(
    () =>
  ...
  // Стабильные ключи строк (без Math.random — иначе виртуализация ремонтирует строки)
  const newPositionIndexByRef = useMemo(() => {
    const map = new Map<ParsedRow, number>();
    newPositions.forEach((pos, idx) => map.set(pos, idx));
    return map;
  }, [newPositions]);

  const rowKey = useCallback(
    (record: MatchPair) => {
      if (record.oldPosition) return record.oldPosition.id;
      if (record.newPosition) {
        return `new-${newPositionIndexByRef.get(record.newPosition) ?? record.newPosition.work_name}`;
      }
      return 'unknown';
    },
    [newPositionIndexByRef]
  );

  const columns: ColumnsType<MatchPair> = useMemo(
    () => [
      {
        title: 'Старая версия',
        key: 'oldPosition',
            ...
            <Space direction="vertical" size="small" style={{ width: '100%' }}>
              ...
              >
                <Badge
                  ...
                  style={{
                    backgroundColor:
                      color === 'green' ? '#52c41a' : color === 'orange' ? '#faad14' : '#ff4d4f',
                  }}
                >
    ...
    [newPositionOptions, onManualMatch, onToggleTransfer, onBreakMatch]
  );
...
export const MatchingTable = memo(MatchingTableInner);
