...
- Варианты: (1) выключить registration/reset в UI/Supabase; (2) только login;
  (3) sync Supabase Auth→Yandex; (4) ускорить app-auth.
- **Статус: ✅ RESOLVED — решение оператора (bridge-период до Go app-auth):**
  - Supabase Auth остаётся источником JWT;
  - **логин существующих пользователей — разрешён**;
  - **новые регистрации — временно запрещены**;
  - **password reset — временно запрещён** или выполняется **только вручную
    оператором**;
  - **смена email / пароля пользователями — временно запрещена**;
  - полный переход на **Go app-auth — отдельный следующий этап**.
- ⚠️ **Это временный bridge-mode.** Ограничения снимаются только после
  внедрения Go app-auth; до тех пор любые auth-изменения вне разрешённого
  login-сценария не синхронизируются Supabase↔Yandex и должны быть закрыты на
  уровне UI/Supabase.
- Follow-up (отдельные последующие этапы, вне cutover):
  1. реализовать **Go app-auth** (login/refresh/forgot/reset, собственный
     JWT issuer + JWKS, `app_auth` password store);
  2. перевести **frontend** с Supabase Auth на Go auth;
  3. **удалить Supabase runtime** (Auth/SDK/project) — отдельным этапом после
     стабилизации app-auth.
...
- Статус: **OPEN** — стабильный prod-путь и том не настроены. Детали,
  non-secret mount-template и open-checklist —
  [21_PRODUCTION_ENV_READINESS.md](./21_PRODUCTION_ENV_READINESS.md).
...
| Auth drift decision зафиксирован (login only; registration/reset/email/password change disabled до app-auth) | ✅ RESOLVED (оператор) | оператор |
| Production CA на стабильном пути + том в compose (см. 21_PRODUCTION_ENV_READINESS.md) | ☐ OPEN | DevOps |
