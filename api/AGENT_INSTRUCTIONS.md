...
**Если ходишь HTTP напрямую — заголовок ровно один: `X-API-Key: thk_…`.**
`Authorization: Bearer` не отправляй вообще: это путь для сессии человека, и ключ,
посланный так, получит `401 invalid or expired token` при полностью исправном ключе.
Список тендеров для ключа — `GET /api/v1/tenders/brief`; маршрут `GET /api/v1/tenders`
ключу не открыт.
...
| Код | Что делать |
|---|---|
| 401 `invalid or expired token` | заголовок не тот: убрать `Authorization: Bearer`, слать `X-API-Key`. Ключ исправен — **новый не просить**, ответ будет тот же |
| 401 `invalid API key` | вот теперь ключ: отозван или просрочен — попросить выпустить новый в «Настройки → Доступ к API» |
| 404 на верном id | маршрута нет в развёрнутой сборке — сказать об этом, не подбирать другие пути |
